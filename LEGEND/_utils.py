...
def load_simulated_data(
    n_genes: int = 15000, n_celltype: int = 5, n_observations: int = 1000
) -> ad.AnnData:
    """
    Gaussian Blobs.
    ...
    adata = sc.datasets.blobs(
        n_variables=n_genes, n_centers=n_celltype, n_observations=n_observations
    )
    adata.X[adata.X < 0] = 0
    adata.X[adata.X > 5] = 0
...
def load_mouse_cortex(min_genes: int = 200, min_cells: int = 3) -> ad.AnnData:
    if os.path.exists("data/sc_mouse_cortex.h5ad"):
        adata = ad.read_h5ad("data/sc_mouse_cortex.h5ad")
    else:
...
def set_logger(
    verbosity: Literal[0, 1, 2] = 1, log_path: Optional[Union[os.PathLike, str]] = None
):
    """
    ...
    if log_path:
        ...
        logger.add(
            sys.stdout, colorize=True, level=level_dict[verbosity], format=formatter
        )
