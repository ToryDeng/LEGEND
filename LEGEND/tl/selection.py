...
def select_from_clusters(
    ...
    if version == "fast":
        is_single_cluster = adata.var["cluster"].isin(
            gene_cluster_counts[gene_cluster_counts == 1].index
        )  # (n_genes,)
        if post_hoc_filtering and (n_single_clusters := is_single_cluster.sum()) > 0:  # if contains single clusters
            ...
            is_outlier = (
                IsolationForest(random_state=random_state).fit_predict(deviances.reshape(-1, 1)) == -1
            )  # (n_single_genes,)
        ...
        grouped = (
            adata.var.loc[~is_single_cluster, ("cluster", "closeness")]
            .rename_axis("gene")
            .groupby(by="cluster")["closeness"]
        )
...
def select_largest_from_groups(grouped: pd.core.groupby.DataFrameGroupBy, modality: str, rel_pct: int = 20):
    ...
    if have_modality_rel.sum() == 2:  # LEGEND
        ...
        largest_genes = (
            grouped["relevance"]
            .apply(lambda x: x.nlargest(max(np.ceil(len(x) * rel_pct / 100).astype(int), 1), "all"))
            .index.get_level_values(1)
        )
        logger.opt(colors=True).debug(
            f"Detected <yellow>{largest_genes.shape[0]}</yellow> genes using combined relevance."
        )

        rna_genes = (
            grouped["relevance_rna"]
            .apply(lambda x: x.nlargest(max(np.ceil(len(x) * rel_pct / 100).astype(int), 1), "all"))
            .index.get_level_values(1)
        )
        ...
        if modality == "sc":  # scGeneClust
            largest_genes = grouped["relevance"].apply(lambda x: x.nlargest(1, "all")).index.get_level_values(1)
        else:  # LEGEND-no-sc
            largest_genes = (
                grouped["relevance"]
                .apply(lambda x: x.nlargest(max(np.ceil(len(x) * rel_pct / 100).astype(int), 1), "all"))
